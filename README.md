...
- 动作清单包含 1324 条元数据；自用本地 Debug 构建直接使用本机已有的第三方 GIF，公共或商业发布仍需单独完成权利审核。
...
- 记录组数、次数、重量和训练感受。
- 支持在自用本地构建中查看动作 GIF，并提供休息计时、动作替换和跳过动作。
- 支持根据历史训练生成下一次训练调整建议。
...
- 自用本地 Debug 构建不因媒体授权记录阻挡 GIF 打包或展示；公共或商业构建不得据此推断已获得发布授权。
